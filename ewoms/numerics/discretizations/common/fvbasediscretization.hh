...
SET_TYPE_PROP(FvBaseDiscretization, VertexMapper,
              Dune::MultipleCodimMultipleGeomTypeMapper<GET_PROP_TYPE(TypeTag, GridView)>);
#else
SET_TYPE_PROP(FvBaseDiscretization, VertexMapper,
              Dune::MultipleCodimMultipleGeomTypeMapper<GET_PROP_TYPE(TypeTag, GridView), Dune::MCMGVertexLayout>);
#endif
...
#else
SET_TYPE_PROP(FvBaseDiscretization, ElementMapper,
              Dune::MultipleCodimMultipleGeomTypeMapper<GET_PROP_TYPE(TypeTag, GridView), Dune::MCMGElementLayout>);
#endif
...
public:
    using type = Ewoms::Linear::NullBorderListCreator<GridView, DofMapper>;
};
...
SET_TYPE_PROP(FvBaseDiscretization, EqVector,
              Dune::FieldVector<GET_PROP_TYPE(TypeTag, Scalar),
...
SET_TYPE_PROP(FvBaseDiscretization, ElementEqVector,
              Dune::BlockVector<GET_PROP_TYPE(TypeTag, EqVector)>);
...
SET_TYPE_PROP(FvBaseDiscretization, SolutionVector,
              Dune::BlockVector<GET_PROP_TYPE(TypeTag, PrimaryVariables)>);
...
class FvBaseDiscretization
{
    ...
    using GridCommHandleFactory = GET_PROP_TYPE(TypeTag, GridCommHandleFactory);
    using NewtonMethod = GET_PROP_TYPE(TypeTag, NewtonMethod);
    using LocalLinearizer = GET_PROP_TYPE(TypeTag, LocalLinearizer);
    ...
    using IntensiveQuantitiesVector = std::vector<IntensiveQuantities, Ewoms::aligned_allocator<IntensiveQuantities, alignof(IntensiveQuantities)> >;
    ...
    public:
        BlockVectorWrapper(const std::string& name EWOMS_UNUSED, const size_t size)
            : blockVector_(size)
        {}
...
#if HAVE_DUNE_FEM
    using DiscreteFunctionSpace = GET_PROP_TYPE(TypeTag, DiscreteFunctionSpace)   ;
...
public:
     ...
     */
    void prefetch(const Element& elem EWOMS_UNUSED) const
    {
     ...
     */
    void checkConservativeness(Scalar EWOMS_OPTIM_UNUSED tolerance = -1, bool EWOMS_OPTIM_UNUSED verbose=false) const
    {
#ifndef NDEBUG
  ...
  public:
     ...
     */
    Scalar eqWeight(unsigned globalVertexIdx EWOMS_UNUSED, unsigned eqIdx EWOMS_UNUSED) const
    { return 1.0; }
     ...
     */
    void updatePVWeights(const ElementContext& elemCtx EWOMS_UNUSED) const
    { }
...
protected:
    ...
    void supplementInitialSolution_(PrimaryVariables& priVars EWOMS_UNUSED,
