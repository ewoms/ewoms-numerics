...
public:
        ...
        -> typename std::remove_reference<decltype(quantityCallback.operator()(0))>::type
    {
        ...
        if (i == focusDofIdx)
            ...
            value = Ewoms::getValue(quantityCallback(i))*interiorDistance;

        if (j == focusDofIdx)
            value += quantityCallback(j)*exteriorDistance;
        else
            value += Ewoms::getValue(quantityCallback(j))*exteriorDistance;
        ...
        -> typename std::remove_reference<decltype(quantityCallback.operator()(0))>::type
    {
        ...
        if (i == focusDofIdx) {
            ...
            const auto& dofVal = Ewoms::getValue(quantityCallback(i));
            for (int k = 0; k < dofVal.size(); ++k)
                value[k] = Ewoms::getValue(dofVal[k])*interiorDistance;
        }
        ...
        if (j == focusDofIdx) {
            ...
            const auto& dofVal = quantityCallback(j);
            for (int k = 0; k < dofVal.size(); ++k)
                value[k] += Ewoms::getValue(dofVal[k])*exteriorDistance;
        }
                           ...
                           const QuantityCallback& quantityCallback) const
    {
        ...
        if (i == focusIdx) {
            deltay =
                Ewoms::getValue(quantityCallback(j))
                - quantityCallback(i);
        }
        else if (j == focusIdx) {
            deltay =
                quantityCallback(j)
                - Ewoms::getValue(quantityCallback(i));
        }
        else
            deltay =
                Ewoms::getValue(quantityCallback(j))
     ...
     */
    template <class QuantityCallback>
    auto calculateBoundaryValue(const ElementContext& elemCtx EWOMS_UNUSED,
                                   ...
                                   const QuantityCallback& quantityCallback) const
    {
        ...
        if (face.interiorIndex() == elemCtx.focusDofIndex())
            ...
            deltay =
                Ewoms::getValue(quantityCallback.boundaryValue())
                - Ewoms::getValue(quantityCallback(face.interiorIndex()));
