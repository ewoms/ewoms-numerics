...
public:
        ...
        if (useVolumetricResidual) {
            ...
            for (unsigned dofIdx=0; dofIdx < numDof; ++dofIdx) {
                if (elemCtx.dofTotalVolume(dofIdx, /*timeIdx=*/0) > 0.0) {
                    ...
                    Ewoms::Valgrind::CheckDefined(dofVolume);
                     ...
                     unsigned timeIdx) const
    {
        ...
        for (unsigned dofIdx=0; dofIdx < numPrimaryDof; dofIdx++) {
            for (unsigned eqIdx = 0; eqIdx < numEq; ++eqIdx) {
                Ewoms::Valgrind::CheckDefined(storage[dofIdx][eqIdx]);
                assert(Ewoms::isfinite(storage[dofIdx][eqIdx]));
            }
        }
#endif
    }
                    ...
                    unsigned timeIdx) const
    {
        ...
        for (unsigned i=0; i < numDof; i++) {
            for (unsigned j = 0; j < numEq; ++ j) {
                assert(Ewoms::isfinite(residual[i][j]));
                Ewoms::Valgrind::CheckDefined(residual[i][j]);
            }
        }
#endif
     ...
     */
    void computeStorage(EqVector& storage EWOMS_UNUSED,
     ...
     */
    void computeFlux(RateVector& flux EWOMS_UNUSED,
                     const ElementContext& elemCtx EWOMS_UNUSED,
     ...
     */
    void computeSource(RateVector& source EWOMS_UNUSED,
                       const ElementContext& elemCtx EWOMS_UNUSED,
                       unsigned dofIdx EWOMS_UNUSED,
                       unsigned timeIdx EWOMS_UNUSED) const
    {
...
protected:
                              ...
                              unsigned timeIdx) const
    {
        ...
        Ewoms::Valgrind::SetUndefined(values);
        ...
        for (unsigned eqIdx = 0; eqIdx < values.size(); ++eqIdx)  {
            ...
            Ewoms::Valgrind::CheckDefined(values[eqIdx]);
            assert(Ewoms::isfinite(values[eqIdx]));
        }
                          ...
                          ElementContext& elemCtx) const
    {
        ...
        for (unsigned dofIdx=0; dofIdx < numPrimaryDof; dofIdx++) {
            ...
            Ewoms::Valgrind::CheckDefined(extrusionFactor);
            assert(Ewoms::isfinite(extrusionFactor));
            assert(extrusionFactor > 0.0);
            Scalar scvVolume =
               elemCtx.stencil(/*timeIdx=*/0).subControlVolume(dofIdx).volume() * extrusionFactor;
            Ewoms::Valgrind::CheckDefined(scvVolume);
            assert(Ewoms::isfinite(scvVolume));
...
#ifndef NDEBUG
            Ewoms::Valgrind::CheckDefined(tmp);
            for (unsigned eqIdx = 0; eqIdx < numEq; ++eqIdx)
                assert(Ewoms::isfinite(tmp[eqIdx]));
#endif
            ...
            if (elemCtx.enableStorageCache()) {
                    ...
                    !elemCtx.haveStashedIntensiveQuantities())
                {
                    ...
                    Ewoms::Valgrind::CheckDefined(tmp2);
            ...
            Ewoms::Valgrind::CheckDefined(residual[dofIdx]);
            ...
            {
                for (unsigned eqIdx = 0; eqIdx < numEq; ++eqIdx)
                    residual[dofIdx][eqIdx] -= Ewoms::scalarValue(sourceRate[eqIdx])*scvVolume;
            }
            else {
                for (unsigned eqIdx = 0; eqIdx < numEq; ++eqIdx) {
