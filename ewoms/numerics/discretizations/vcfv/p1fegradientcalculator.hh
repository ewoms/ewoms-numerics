...
#include <ewoms/numerics/discretizations/common/fvbasegradientcalculator.hh>
...
#else
#define EWOMS_NO_LOCALFUNCTIONS_UNUSED  EWOMS_UNUSED
#endif
...
public:
        ...
        ->  typename std::remove_reference<typename QuantityCallback::ResultType>::type
    {
        if (GET_PROP_VALUE(TypeTag, UseP1FiniteElementGradients)) {
            ...
            typedef Ewoms::MathToolbox<QuantityType> Toolbox;
        ...
        ->  typename std::remove_reference<typename QuantityCallback::ResultType>::type
    {
        if (GET_PROP_VALUE(TypeTag, UseP1FiniteElementGradients)) {
            ...
            typedef Ewoms::MathToolbox<FieldType> Toolbox;
        ...
        if (GET_PROP_VALUE(TypeTag, UseP1FiniteElementGradients)) {
            ...
            for (unsigned vertIdx = 0; vertIdx < elemCtx.numDof(/*timeIdx=*/0); ++vertIdx) {
                    ...
                    elemCtx.focusDofIndex() == vertIdx)
                {
                    ...
                    const auto& tmp = p1Gradient_[fapIdx][vertIdx];
                    for (int dimIdx = 0; dimIdx < dim; ++ dimIdx)
                        quantityGrad[dimIdx] += Ewoms::scalarValue(dofVal)*tmp[dimIdx];
                }
            }
#endif
        }
