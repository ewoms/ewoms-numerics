...
{
    using ConvergenceCriterion = Ewoms::Linear::ConvergenceCriterion<Vector>;
    ...
    bool apply(Vector& x)
    {
        ...
        for (; report_.iterations() < maxIterations_; report_.increment()) {
            ...
            if (std::abs(rho) <= breakdownEps || std::abs(omega) <= breakdownEps)
                throw Ewoms::NumericalIssue("Breakdown of the BiCGStab solver (division by zero)");
            ...
            if (std::abs(alpha) <= breakdownEps)
                throw Ewoms::NumericalIssue("Breakdown of the BiCGStab solver (stagnation detected)");
