...
#include <ewoms/numerics/io/vtkblackoilsolventmodule.hh>
...
public:
    ...
    static void initFromEclState(const Ewoms::EclipseState& eclState, const Schedule& schedule)
    {
        ...
        for (unsigned satRegionIdx = 0; satRegionIdx < numSatRegions; ++ satRegionIdx) {
            const auto& ssfnTable = ssfnTables.template getTable<Ewoms::SsfnTable>(satRegionIdx);
        ...
        if (!eclState.getTableManager().getMiscTables().empty()) {
            ...
            if (!sof2Tables.empty()) {
                ...
                for (unsigned satRegionIdx = 0; satRegionIdx < numSatRegions; ++ satRegionIdx) {
                    const auto& sof2Table = sof2Tables.template getTable<Ewoms::Sof2Table>(satRegionIdx);
            ...
            if (!miscTables.empty()) {
                ...
                for (unsigned miscRegionIdx = 0; miscRegionIdx < numMiscRegions; ++miscRegionIdx) {
                    const auto& miscTable = miscTables.template getTable<Ewoms::MiscTable>(miscRegionIdx);
            ...
            if (!pmiscTables.empty()) {
                ...
                for (unsigned regionIdx = 0; regionIdx < numMiscRegions; ++regionIdx) {
                    const auto& pmiscTable = pmiscTables.template getTable<Ewoms::PmiscTable>(regionIdx);
            ...
            if (!msfnTables.empty()) {
                ...
                for (unsigned regionIdx = 0; regionIdx < numSatRegions; ++regionIdx) {
                    const Ewoms::MsfnTable& msfnTable = msfnTables.template getTable<Ewoms::MsfnTable>(regionIdx);
            ...
            if (!sorwmisTables.empty()) {
                ...
                for (unsigned regionIdx = 0; regionIdx < numMiscRegions; ++regionIdx) {
                    const auto& sorwmisTable = sorwmisTables.template getTable<Ewoms::SorwmisTable>(regionIdx);
            ...
            if (!sgcwmisTables.empty()) {
                ...
                for (unsigned regionIdx = 0; regionIdx < numMiscRegions; ++regionIdx) {
                    const auto& sgcwmisTable = sgcwmisTables.template getTable<Ewoms::SgcwmisTable>(regionIdx);
            ...
            if (!eclState.getTableManager().getTlpmixpaTables().empty()) {
                ...
                if (!tlpmixparTables.empty()) {
                    ...
                    for (unsigned regionIdx = 0; regionIdx < numMiscRegions; ++regionIdx) {
                        const auto& tlpmixparTable = tlpmixparTables.template getTable<Ewoms::TlpmixpaTable>(regionIdx);
...

    {
        ...
        if (blackoilConserveSurfaceVolume) {
            if (upIdx == inIdx)
                ...
                flux[contiSolventEqIdx] =
                        extQuants.solventVolumeFlux()
                        *Ewoms::decay<Scalar>(up.solventInverseFormationVolumeFactor());
        }
        else {
            if (upIdx == inIdx)
                flux[contiSolventEqIdx] =
                ...
                flux[contiSolventEqIdx] =
                        extQuants.solventVolumeFlux()
                        *Ewoms::decay<Scalar>(up.solventDensity());
        }
    }
...
public:
    ...
    void solventPostSatFuncUpdate_(const ElementContext& elemCtx EWOMS_UNUSED,
                                   unsigned scvIdx EWOMS_UNUSED,
                                   unsigned timeIdx EWOMS_UNUSED)
    { }

    void solventPvtUpdate_(const ElementContext& elemCtx EWOMS_UNUSED,
                           unsigned scvIdx EWOMS_UNUSED,
                           unsigned timeIdx EWOMS_UNUSED)
    { }
...
public:
                              ...
                              unsigned timeIdx)
    {
        ...
        gradCalc.calculateGradient(solventPGrad,
                                   ...
                                   scvfIdx,
                                   pressureCallback);
        Ewoms::Valgrind::CheckDefined(solventPGrad);
        ...
        if (EWOMS_GET_PARAM(TypeTag, bool, EnableGravity)) {
            ...
            for (unsigned dimIdx = 0; dimIdx < dimWorld; ++dimIdx) {
                ...
                if (!Ewoms::isfinite(solventPGrad[dimIdx]))
                    throw Ewoms::NumericalIssue("Non-finite potential gradient for solvent 'phase'");
            }
        }
        ...
        if (solventUpstreamDofIdx_ == i)
            ...
            solventVolumeFlux_ = solventPGradNormal*Ewoms::scalarValue(up.solventMobility());
    }
                               ...
                               unsigned timeIdx)
    {
        ...
        if (std::abs(Ewoms::scalarValue(pressureDiffSolvent)) > thpres) {
        ...
        if (solventUpstreamDofIdx_ == interiorDofIdx)
            ...
            solventVolumeFlux_ =
                Ewoms::scalarValue(up.solventMobility())
                *(-trans/faceArea)
                *pressureDiffSolvent;
    }
...
public:
    ...
    void updateVolumeFluxTrans(const ElementContext& elemCtx EWOMS_UNUSED,
                              unsigned scvfIdx EWOMS_UNUSED,
                              unsigned timeIdx EWOMS_UNUSED)
    { }
