...
#include <ewoms/numerics/io/vtkblackoilpolymermodule.hh>
...
public:
    ...
    static void initFromDeck(const Ewoms::Deck& deck, const Ewoms::EclipseState& eclState)
    {
        ...
        if (!plyrockTables.empty()) {
            ...
            for (unsigned satRegionIdx = 0; satRegionIdx < numSatRegions; ++ satRegionIdx) {
                const auto& plyrockTable = plyrockTables.template getTable<Ewoms::PlyrockTable>(satRegionIdx);
        ...
        if (!plyadsTables.empty()) {
            ...
            for (unsigned satRegionIdx = 0; satRegionIdx < numSatRegions; ++ satRegionIdx) {
                const auto& plyadsTable = plyadsTables.template getTable<Ewoms::PlyadsTable>(satRegionIdx);
                // Copy data
        ...
        if (!plyviscTables.empty()) {
            ...
            if (enablePolymerMolarWeight) {
                Ewoms::OpmLog::warning("PLYVISC should not be used in POLYMW runs, "
                ...
                for (unsigned pvtRegionIdx = 0; pvtRegionIdx < numPvtRegions; ++ pvtRegionIdx) {
                    const auto& plyadsTable = plyviscTables.template getTable<Ewoms::PlyviscTable>(pvtRegionIdx);
                    // Copy data
                    const auto& c = plyadsTable.getPolymerConcentrationColumn();
        ...
        if (!plymaxTables.empty()) {
            for (unsigned mixRegionIdx = 0; mixRegionIdx < numMixRegions; ++ mixRegionIdx) {
                const auto& plymaxTable = plymaxTables.template getTable<Ewoms::PlymaxTable>(mixRegionIdx);
        ...
        if (deck.hasKeyword("PLMIXPAR")) {
            if (enablePolymerMolarWeight) {
                Ewoms::OpmLog::warning("PLMIXPAR should not be used in POLYMW runs, it will have no effect.\n");
            }
            else {
        ...
        if ((hasPlyshlog_ || hasShrate_) && enablePolymerMolarWeight) {
            Ewoms::OpmLog::warning("PLYSHLOG and SHRATE should not be used in POLYMW runs, they will have no effect.\n");
        }
        ...
        if (hasPlyshlog_ && !enablePolymerMolarWeight) {
            ...
            for (unsigned pvtRegionIdx = 0; pvtRegionIdx < numPvtRegions; ++ pvtRegionIdx) {
                const auto& plyshlogTable = plyshlogTables.template getTable<Ewoms::PlyshlogTable>(pvtRegionIdx);
        ...
        if (enablePolymerMolarWeight) {
            const Ewoms::DeckKeyword& plyvmhKeyword = deck.getKeyword("PLYVMH");
            ...
            if (plyvmhKeyword.size() > 0) {
                for (size_t regionIdx = 0; regionIdx < plyvmhKeyword.size(); ++regionIdx) {
                    const Ewoms::DeckRecord& record = plyvmhKeyword.getRecord(regionIdx);
    ...
    */
                           ...
                           const IntensiveQuantities& intQuants)
    {
        ...
        if (enablePolymerMolarWeight) {
            accumulationPolymer = Ewoms::max(accumulationPolymer, 1e-10);
...

    {
        ...
        if (upIdx == inIdx) {
            ...
            flux[contiPolymerEqIdx] =
                    ...
                    *Ewoms::decay<Scalar>(up.polymerViscosityCorrection())
                    /Ewoms::decay<Scalar>(extQuants.polymerShearFactor())
                    *Ewoms::decay<Scalar>(up.polymerConcentration());

            // modify water
            flux[contiWaterEqIdx] /=
                    Ewoms::decay<Scalar>(extQuants.waterShearFactor());
        }
        ...
        if (enablePolymerMolarWeight) {
            if (upIdx == inIdx)
                ...
                flux[contiPolymerMolarWeightEqIdx] =
                    flux[contiPolymerEqIdx]*Ewoms::decay<Scalar>(up.polymerMoleWeight());
        }
                                         ...
                                         const Evaluation& v0)
    {
        ...
        auto v0AbsLog = Ewoms::log(Ewoms::abs(v0));
        ...
        for (size_t i = 0; i < numTableEntries; ++i) {
            ...
            shearEffectMultiplier[i] = Ewoms::log(shearEffectMultiplier[i]);
        }
        ...
        for (int i = 0; i < 20; ++i) {
            ...
            auto df = dF(u);
            u -= f/df;
            if (std::abs(Ewoms::scalarValue(f)) < 1e-12) {
                converged = true;
                break;
            }
        }
        ...
        return Ewoms::exp(logShearEffectMultiplier.eval(u, /*extrapolate=*/true));
...
public:
    ...
    void updateShearMultipliersPerm(const ElementContext& elemCtx EWOMS_UNUSED,
                                ...
                                unsigned timeIdx)
    {
        ...
        Evaluation denom = Ewoms::max(poroAvg * (Sw - Swcr), 1e-12);
        ...
        if (PolymerModule::hasShrate()) {
            ...
            if (trans > 0.0) {
                ...
                waterVolumeVelocity *=
                    PolymerModule::shrate(pvtnumRegionIdx)*Ewoms::sqrt(poroAvg*Sw / (relWater*absPerm));
                assert(Ewoms::isfinite(waterVolumeVelocity));
            }
        }
