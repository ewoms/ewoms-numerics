...
public:
        ...
        for (unsigned phaseIdx = 0; phaseIdx < numPhases; ++phaseIdx) {
            if (!FluidSystem::phaseIsActive(phaseIdx)) {
                if (Indices::numPhases == 3) { // add trivial equation for the pseudo phase
                    ...
                    if (timeIdx == 0)
                        storage[conti0EqIdx + activeCompIdx] = Ewoms::variable<LhsEval>(0.0, conti0EqIdx + activeCompIdx);
                    else
    ...
    {
        const auto& invB = Ewoms::getInvB_<FluidSystem, FluidState, UpEval>(upFs, phaseIdx, pvtRegionIdx);
        ...
        if (phaseIdx == oilPhaseIdx) {
            ...
            if (FluidSystem::enableDissolvedGas()) {
                const auto& Rs = Ewoms::BlackOil::getRs_<FluidSystem, FluidState, UpEval>(upFs, pvtRegionIdx);
            ...
            if (FluidSystem::enableVaporizedOil()) {
                const auto& Rv = Ewoms::BlackOil::getRv_<FluidSystem, FluidState, UpEval>(upFs, pvtRegionIdx);
