...
template <class TypeTag, bool enableSsaSolvent>
...
class BlackOilPrimaryVariables : public FvBasePrimaryVariables<TypeTag>
{
    ...
    using SsaSolventModule = BlackOilSsaSolventModule<TypeTag, enableSsaSolvent>;
...
public:
    ...
    BlackOilPrimaryVariables(Scalar value)
        : ParentType(value)
    {
        Ewoms::Valgrind::SetUndefined(primaryVarsMeaning_);
        pvtRegionIdx_ = 0;
    }
                                ...
                                bool isInEquilibrium = false)
    {
        ...
        for (unsigned phaseIdx = 1; phaseIdx < numPhases; ++phaseIdx) {
            Ewoms::Valgrind::CheckDefined(fluidState.temperature(0));
            Ewoms::Valgrind::CheckDefined(fluidState.temperature(phaseIdx));
    ...
    void assignNaive(const FluidState& fluidState)
    {
        ...
        if (primaryVarsMeaning() == OnePhase_p) {
            ...
            const auto& Rs = Ewoms::BlackOil::getRs_<FluidSystem, FluidState, Scalar>(fluidState, pvtRegionIdx_);
            ...
            const auto& Rv = Ewoms::BlackOil::getRv_<FluidSystem, FluidState, Scalar>(fluidState, pvtRegionIdx_);
            if (waterEnabled)
                (*this)[waterSaturationIdx] = FsToolbox::value(fluidState.saturation(waterPhaseIdx));
    ...
    bool adaptPrimaryVariables(const Problem& problem, unsigned globalDofIdx, Scalar eps = 0.0)
    {
        ...
        if (primaryVarsMeaning() == Sw_po_Sg) {
            ...
            if (Sg < -eps && So2 > 0.0 && FluidSystem::enableDissolvedGas()) {
                ...
                Scalar RsSat = enableSsaSolvent ? SsaSolventModule::rs(pvtRegionIndex(),
                                                             po,
                                                             zFraction_())
            ...
            if (So < -eps && Sg2 > 0.0 && FluidSystem::enableVaporizedOil()) {
                ...
                Scalar RvSat = enableSsaSolvent ? SsaSolventModule::rv(pvtRegionIndex(),
                                                             pg,
                                                             zFraction_())
    ...
    void checkDefined() const
    {
        ...
        Ewoms::Valgrind::CheckDefined(primaryVarsMeaning_);
