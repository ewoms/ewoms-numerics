...
protected:
                             ...
                             unsigned timeIdx)
    {
        ...
        if (focusDofIdx == i) {
            ergunCoefficient_ =
                (intQuantsIn.ergunCoefficient() +
                 Ewoms::getValue(intQuantsEx.ergunCoefficient()))/2;
        }
        else if (focusDofIdx == j)
            ergunCoefficient_ =
                (Ewoms::getValue(intQuantsIn.ergunCoefficient()) +
        ...
        for (unsigned phaseIdx=0; phaseIdx < numPhases; phaseIdx++) {
            ...
            if (focusDofIdx == upIdx) {
                ...
                density_[phaseIdx] =
                    Ewoms::getValue(up.fluidState().density(phaseIdx));
                mobilityPassabilityRatio_[phaseIdx] =
                    Ewoms::getValue(up.mobilityPassabilityRatio(phaseIdx));
            }
        }
    }
                                     ...
                                     const FluidState& fluidState)
    {
        ...
        if (focusDofIdx == i)
            ...
            ergunCoefficient_ = Ewoms::getValue(intQuantsIn.ergunCoefficient());
        ...
        for (unsigned phaseIdx=0; phaseIdx < numPhases; phaseIdx++) {
            ...
            if (focusDofIdx == i) {
                ...
                density_[phaseIdx] =
                    Ewoms::getValue(intQuantsIn.fluidState().density(phaseIdx));
                mobilityPassabilityRatio_[phaseIdx] =
                    Ewoms::getValue(intQuantsIn.mobilityPassabilityRatio(phaseIdx));
            }
        }
    }
    ...
    void calculateFluxes_(const ElementContext& elemCtx, unsigned scvfIdx, unsigned timeIdx)
    {
        ...
        Ewoms::Valgrind::CheckDefined(normal);
        ...
        if (focusDofIdx == i)
            ergunCoefficient_ =
                (intQuantsI.ergunCoefficient() +
                 Ewoms::getValue(intQuantsJ.ergunCoefficient())) / 2;
        else if (focusDofIdx == j)
            ergunCoefficient_ =
                (Ewoms::getValue(intQuantsI.ergunCoefficient()) +
    ...
    void calculateForchheimerFlux_(unsigned phaseIdx)
    {
        ...
        while (deltaV.one_norm() > 1e-11) {
            if (newtonIter >= 50)
                throw Ewoms::NumericalIssue("Could not determine Forchheimer velocity within "
    ...
    void forchheimerResid_(DimEvalVector& residual, unsigned phaseIdx) const
    {
        ...
        Ewoms::Valgrind::CheckDefined(residual);
    }
