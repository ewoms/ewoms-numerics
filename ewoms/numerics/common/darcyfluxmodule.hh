...
protected:
                             ...
                             unsigned timeIdx)
    {
        ...
        for (unsigned phaseIdx = 0; phaseIdx < numPhases; ++phaseIdx) {
            if (!elemCtx.model().phaseIsConsidered(phaseIdx)) {
                Ewoms::Valgrind::SetUndefined(potentialGrad_[phaseIdx]);
                continue;
            }
            ...
            gradCalc.calculateGradient(potentialGrad_[phaseIdx],
                                       ...
                                       faceIdx,
                                       pressureCallback);
            Ewoms::Valgrind::CheckDefined(potentialGrad_[phaseIdx]);
        }
        ...
        if (EWOMS_GET_PARAM(TypeTag, bool, EnableGravity)) {
            ...
            for (unsigned phaseIdx=0; phaseIdx < numPhases; phaseIdx++) {
                ...
                for (unsigned dimIdx = 0; dimIdx < potentialGrad_[phaseIdx].size(); ++dimIdx) {
                    if (!Ewoms::isfinite(potentialGrad_[phaseIdx][dimIdx])) {
                        throw Ewoms::NumericalIssue("Non-finite potential gradient for phase '"
                                     ...
                                     const FluidState& fluidState)
    {
        ...
        if (EWOMS_GET_PARAM(TypeTag, bool, EnableGravity)) {
            ...
            for (unsigned phaseIdx=0; phaseIdx < numPhases; phaseIdx++) {
                ...
                Ewoms::Valgrind::CheckDefined(pStatIn);
        ...
        Ewoms::Valgrind::CheckDefined(kr);
    ...
    void calculateFluxes_(const ElementContext& elemCtx, unsigned scvfIdx, unsigned timeIdx)
    {
        ...
        for (unsigned phaseIdx=0; phaseIdx < numPhases; phaseIdx++) {
            ...
            Ewoms::Valgrind::CheckDefined(filterVelocity_[phaseIdx]);
    ...
    {
#ifndef NDEBUG
        assert(Ewoms::isfinite(mobility_[phaseIdx]));
        ...
        for (unsigned i = 0; i < filterVelocity_[phaseIdx].size(); ++ i)
            assert(Ewoms::isfinite(filterVelocity_[phaseIdx][i]));
#endif
    }
