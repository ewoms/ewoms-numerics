...
#include <ewoms/numerics/discretizations/common/fvbaseextensivequantities.hh>
...
{
    using ParentType = GET_PROP_TYPE(TypeTag, DiscExtensiveQuantities);
...
public:
    ...
    void update(const ElementContext& elemCtx, unsigned scvfIdx, unsigned timeIdx)
    {
        ...
        for (unsigned phaseIdx = 0; phaseIdx < numPhases; ++phaseIdx) {
            if (!elemCtx.model().phaseIsConsidered(phaseIdx)) {
                Ewoms::Valgrind::SetUndefined(upstreamScvIdx_[phaseIdx]);
                Ewoms::Valgrind::SetUndefined(downstreamScvIdx_[phaseIdx]);
                continue;
            }
     ...
     */
    Scalar upstreamWeight(unsigned phaseIdx EWOMS_UNUSED) const
    { return 1.0; }
