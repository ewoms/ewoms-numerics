...
public:
    ...
    void processElement(const ElementContext& elemCtx)
    {
        ...
        for (unsigned i = 0; i < elemCtx.numPrimaryDof(/*timeIdx=*/0); ++i) {
            ...
            if (porosityOutput_()) porosity_[I] = Ewoms::getValue(intQuants.porosity());
            ...
            for (unsigned phaseIdx = 0; phaseIdx < numPhases; ++phaseIdx) {
                ...
                if (densityOutput_())
                    density_[phaseIdx][I] = Ewoms::getValue(fs.density(phaseIdx));
                if (saturationOutput_())
                    saturation_[phaseIdx][I] = Ewoms::getValue(fs.saturation(phaseIdx));
                if (mobilityOutput_())
                    mobility_[phaseIdx][I] = Ewoms::getValue(intQuants.mobility(phaseIdx));
                if (relativePermeabilityOutput_())
                    relativePermeability_[phaseIdx][I] = Ewoms::getValue(intQuants.relativePermeability(phaseIdx));
                if (viscosityOutput_())
                    viscosity_[phaseIdx][I] = Ewoms::getValue(fs.viscosity(phaseIdx));
                if (averageMolarMassOutput_())
                    averageMolarMass_[phaseIdx][I] = Ewoms::getValue(fs.averageMolarMass(phaseIdx));
            }
        }
            ...
            for (unsigned faceIdx = 0; faceIdx < elemCtx.numInteriorFaces(/*timeIdx=*/0); ++ faceIdx) {
                ...
                for (unsigned phaseIdx = 0; phaseIdx < numPhases; ++phaseIdx) {
                    ...
                    for (unsigned dimIdx = 0; dimIdx < dimWorld; ++dimIdx)
                        pGrad[dimIdx] = Ewoms::getValue(inputPGrad[dimIdx])*weight;
            ...
            for (unsigned faceIdx = 0; faceIdx < elemCtx.numInteriorFaces(/*timeIdx=*/0); ++ faceIdx) {
                ...
                for (unsigned phaseIdx = 0; phaseIdx < numPhases; ++phaseIdx) {
                    Scalar weight = std::max<Scalar>(1e-16,
                                                     std::abs(Ewoms::getValue(extQuants.volumeFlux(phaseIdx))));
