...
protected:
     ...
     */
    void begin_(const SolutionVector& u  EWOMS_UNUSED)
    {
        numIterations_ = 0;
    ...
    void beginIteration_()
    {
        ...
        if (!succeeded)
            throw Ewoms::NumericalIssue("pre processing of the problem failed");
                   ...
                   const GlobalEqVector& currentResidual)
    {
        ...
        for (unsigned dofIdx = 0; dofIdx < currentResidual.size(); ++dofIdx) {
            ...
            for (unsigned eqIdx = 0; eqIdx < r.size(); ++eqIdx)
                error_ = Ewoms::max(std::abs(r[eqIdx] * model().eqWeight(dofIdx, eqIdx)), error_);
        }
        ...
        if (error_ > newtonMaxError)
            throw Ewoms::NumericalIssue("Newton: Error "+std::to_string(double(error_))
     ...
     */
    void postSolve_(const SolutionVector& currentSolution EWOMS_UNUSED,
                    const GlobalEqVector& currentResidual EWOMS_UNUSED,
                    GlobalEqVector& solutionUpdate EWOMS_UNUSED)
    {
        ...
        for (unsigned i = 0; i < model.numAuxiliaryModules(); ++i) {
            ...
            if (!succeeded)
                throw Ewoms::NumericalIssue("post processing of an auxilary equation failed");
        }
    }
                 ...
                 const GlobalEqVector& currentResidual)
    {
        ...
        if (!std::isfinite(solutionUpdate.one_norm()))
            throw Ewoms::NumericalIssue("Non-finite update!");
     ...
     */
    void updateConstraintDof_(unsigned globalDofIdx  EWOMS_UNUSED,
     ...
     */
    void endIteration_(const SolutionVector& nextSolution  EWOMS_UNUSED,
                       const SolutionVector& currentSolution  EWOMS_UNUSED)
    {
        ++numIterations_;
        ...
        if (!succeeded)
            throw Ewoms::NumericalIssue("post processing of the problem failed");
